...
export default function DomainCard({ domain, onClick }) {
  ...
  return (
    <motion.div
      layout
      whileHover={{ y: -3, boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.15), 0 10px 10px -5px rgba(0, 0, 0, 0.06)' }}
      transition={{ duration: 0.2 }}
      onClick={onClick}
      className="group cursor-pointer bg-white dark:bg-gray-800 rounded-2xl shadow-sm hover:shadow-lg transition-all duration-200 h-full flex flex-col"
      style={{ border: '1.5px solid #000000', padding: '8px' }}
    >
      {/* Inner Container – light grey border */}
      <div
        className="rounded-xl bg-white dark:bg-gray-900 flex-1 flex flex-col"
        style={{ border: '1px solid #d1d5db', padding: '14px' }}
      >
        {/* Header */}

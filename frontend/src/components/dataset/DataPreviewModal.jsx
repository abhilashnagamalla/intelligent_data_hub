...
export default function DataPreviewModal({ open, onClose, resource, content, viewMode, setViewMode }) {
  ...
  const handleCopy = async () => {
    try {
      await navigator.clipboard.writeText(content || 'No raw content available.');
      setCopied(true);
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
      <motion.div
        ...
        className="w-full max-w-6xl h-[90vh] bg-white dark:bg-gray-900 rounded-2xl shadow-2xl overflow-hidden border-2 border-black"
      >
        ...
        <div className="flex border-b-2 border-black px-5 py-3 gap-3 overflow-x-auto bg-gray-50 dark:bg-gray-800">
          <button onClick={() => setViewMode('table')} className={`rounded-lg border-2 border-black px-4 py-2 text-sm font-semibold ${viewMode === 'table' ? 'bg-primary text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-100'}`}>
            Table View
          </button>
        ...
        <div className="h-full overflow-auto p-5 bg-white dark:bg-gray-900">
          {viewMode === 'table' ? (
            rows.length ? (
              <div className="overflow-auto border-2 border-black rounded-xl">
